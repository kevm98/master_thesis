...
class CustomTerrainImporter:
    ...
    def __init__(self, cfg: CustomTerrainImporterCfg):
    ...
    def _compute_env_origins_curriculum(self, num_envs: int, origins: torch.Tensor) -> torch.Tensor:
        """Compute the origins of the environments ensuring NO duplicate assignments."""
        ...
        self.max_terrain_level = num_rows # max level     
        total_available_patches = num_rows * num_cols

        if num_envs > total_available_patches:
            raise RuntimeError(
                f"[TerrainImporter Error] Not enough unique terrain patches!"
                f"\n\tRequested Envs (Robots): {num_envs}"
                f"\n\tAvailable Patches: {total_available_patches} ({num_rows} rows x {num_cols} cols)"
                f"\n\tSolution: Increase 'num_rows' or 'num_cols' in your Terrain Generator Config."
            )
        
        
        # Normal random spawn
        all_possible_indices = torch.randperm(total_available_patches, device=self.device)
        selected_indices = all_possible_indices[:num_envs]

        # No random spawn
        #selected_indices = torch.arange(num_envs, device=self.device)

        self.terrain_levels = torch.div(selected_indices, num_cols, rounding_mode="floor")
        self.terrain_types = selected_indices % num_cols
