...
parser.add_argument(
    "--agent", type=str, default="sb3_contrib_cfg_entry_point", help="Name of the RL agent configuration entry point."
)
...
# from .utils import process_sb3_cfg
...
def process_sb3_cfg(cfg: dict, num_envs: int) -> dict:
    def update_dict(hyperparams: dict[str, Any], depth: int) -> dict[str, Any]:
        for key, value in hyperparams.items():
            ...
            if key == "share_features_extractor":
                hyperparams[key] = bool(value)

            if key in ["shared_lstm", "enable_critic_lstm"]:
                hyperparams[key] = bool(value)
            ...
            if depth == 0:
                if key in ["learning_rate", "clip_range", "clip_range_vf"]:
                    if isinstance(value, str):
                        if "_" in value:
                            # e.g. "linear_3e-4"
                            _, initial_value = value.split("_", 1)
                            initial_value = float(initial_value)
                            hyperparams[key] = lambda progress_remaining: progress_remaining * initial_value
                        else:
                            # e.g. "3e-4" or "0.2" as a string
                            hyperparams[key] = ConstantSchedule(float(value))
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    # print("sb3 train agent:", agent)
    # print("Policy network: ", agent.policy)
    # print("Value network: ", agent_cfg)
